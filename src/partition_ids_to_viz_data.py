...
def read_knesset_partition_by_party():
    ...
    with open('data/member_names.csv') as f:
        ...
        for row in csv_reader:
            party = row['party']
            idx = int(row['index'])
            member_parties.append(party)

    # generated by votes_to_game.filter_infrequent_voters(1000)
    filtered_indexes = [146, 145, 143, 142, 114, 95, 77, 73, 72, 55, 54, 46, 42, 34, 26, 8, 0]
    for idx in filtered_indexes:
        del member_parties[idx]

    coalitions_by_party = {}
    for idx, party in enumerate(member_parties):
