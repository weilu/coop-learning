...
\subsection{Improvement to the PAC Top Covering Algorithm}
...
At any iteration, the largest $SCC$ needs to be extracted as the stable coalition $S$. Assume otherwise where a small $SCC$ was extracted at iteration $l$, as a result, player $i \in S$ ended up in coalitions $S'$ which is different from $S$. We claim that $S'$ is less preferable than $S$ for some player $i \in S$, because $S'$ is extracted from subsequent iteration $l' > l$, therefore their choice set at $l'$ is less preferable than that at $l$. TODO: make this better

This concludes our proof that the largest $SCC$ procedure produces a strictly core stable partition for top responsive games.
