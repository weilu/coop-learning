...
In this paper, we present the first application of strategic hedonic games models on real-world data.
Our contributions include an algorithmic improvement for discovering PAC stable solutions, which we apply on data from the Israeli parliament (Knesset).  We discover that the PAC stable solutions reflect Knesset members' political positions at large, when compared against ground truth party affiliations.
Moreover, these comparisons also reveals politicians whose voting behaviors are known to deviate from party lines.
Finally, we show empirically that PAC hedonic games compare favorably to both $k$-means clustering and stochastic block model, which do not account for strategic behavior, for uncovering voting patterns.
\end{abstract}
