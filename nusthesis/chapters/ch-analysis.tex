...
In the process of reasoning about our experiment results, we realize the
...
as a result, we need to visually describe partition results from various models
such that it is not only intuitively understandable but also aids our 
qualitative analysis (Section~\ref{sec:partition_visualization}).
...
\paragraph{} The partition produced by the SBM with normal edge weight distribution
...
(selected using the average silhouette method and elbow method respectively, see Section~\ref{sec:k_means_clustering}),
...
\paragraph{Coherence}
We want to see whether the opposition and government coalition party members
are well separated by the different models, resulting in a coherent and
logical map.
